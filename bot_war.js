...
(async () => {
  ...
  const url =
    "https://docs.google.com/spreadsheets/d/1_WdrOVcB9wfziod3liUbtZBMquVpLGByLIFvoSI3TCk/edit?gid=1851224612#gid=1851224612";

  // 2. Waktu eksekusi (Format: HH:MM:SS) - Tambahkan Detik!
  // Contoh: '13:14:00' atau '13:14:30' (jam 13, menit 14, detik 30)
  const targetTime = "11:59:45";

  // 3. TARGET KOLOM & DATA YANG AKAN DI-COPY PASTE (SPAM)
  // Silakan ganti kata-katanya di dalam tanda kutip berikuit
  const cellTujuan1 = "I20";
  const dataCopas1 = "Khairun Nisa, Salwa Assyifa, Aliza Khansa";

  const cellTujuan2 = "J20";
  const dataCopas2 = "";

  const cellTujuan3 = "K20";
  ...
  async function jalankanBot() {
    ...
    // Spam 500 kali putaran dengan kecepatan tanpa jeda
    for (let i = 0; i < 30; i++) {
      console.log(
        `Memulai spam iterasi ke-${i + 1}... PASTE SERENTAK DI ${cellTujuan1}`,
      );
      try {
        ...
        // CTRL+V akan langsung mengisi E22, F22, dan G22
        await page.keyboard.down("Control");
        await page.keyboard.press("v");
        await page.keyboard.up("Control");
      } catch (err) {
        console.log(
          "Error! Halaman mungkin berat/terputus. Mencoba memuat ulang (Refresh)...",
        );
        // Jika error, coba refresh paksa
        try {
    ...
    console.log("SELESAI! Menutup program dalam 10 detik...");
